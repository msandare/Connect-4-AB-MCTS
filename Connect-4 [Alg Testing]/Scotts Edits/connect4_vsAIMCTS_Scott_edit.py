...
def drop_piece(board, row, col, piece): # Add opp piece???
    board[row][col] = piece
...
def winning_move(board,piece): #add opp piece
...
def monte_carlo_tree_search(board, SIMULATIONS, piece,opponent):
...
while total_games != 10:
    ...
    while not game_over:
        ...
        if turn == AI and not game_over:
            ...
            #col, minimax_score = alphaBeta(board, 4, -math.inf, math.inf, True)  #alphabeta solver
            col = monte_carlo_tree_search(board, SIMULATIONS, AI_PEICE,PLAYER_PEICE)
            if is_valid_location(board,col):
                row = get_next_open_row(board, col)
